...
$$F(t, T) = \mathbb{E}^{\mathbb{Q}}[\text{VIX}_T \,|\, \mathcal{F}_t]$$
...
The integral converges because $|\psi(u)| \to 0$ as $u \to \infty$ (the variance term $-u^2\sigma^2/(\cdots)$ in the exponent dominates). In practice, we truncate the integration at some upper limit ($u = 100$ for `BigFloat`, $u = 50$ for `Float64`) and use adaptive Gauss-Kronrod quadrature (`QuadGK` in Julia).
...
$$\text{MAPE}(\kappa, \theta, \sigma, \lambda, \eta) = \frac{1}{N}\sum_{i=1}^N \frac{|C_{\text{model}}(K_i) - C_{\text{market}}(K_i)|}{C_{\text{market}}(K_i)}$$
