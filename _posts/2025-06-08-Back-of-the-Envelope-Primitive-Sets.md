...
<ol>
<li><strong>Primitive means no one divides another.</strong> The primes are one such set. So is $\{6,10,15\}$. Density — the proportion of integers you occupy — oscillates too wildly to pick a winner, so we weigh each $a$ by $1/(a\log a)$ and compare the sums.</li>
<li><strong>A private territory $L_a$.</strong> Start with $a$ and multiply by integers built only from primes at least as large as $a$'s largest prime factor. The density of this set is $\frac1a\prod_{q \lt P(a)}(1-1/q)$.</li>
<li><strong>Primitivity makes the territories disjoint,</strong> so they cannot spend more than density $1$ in total. That is a budget.</li>
<li><strong>Mertens converts the budget into an $f$-bound.</strong> Sieving out primes up to $x$ leaves a proportion about $e^{-\gamma}/\log x$ of the integers. Combined with the budget, every primitive set scores less than $e^\gamma\approx 1.781$. The primes score $1.6366$. About $9\%$ of slack remains.</li>
<li><strong>Compete slot by slot.</strong> Split any set by smallest prime factor. If each slot $A_p$ scores at most $f(p)=1/(p\log p)$, the primes win. A crude comparison already does this for the first $10^8$ odd primes, but not for $2$, and not for infinitely many large primes.</li>
<li><strong>Composites convert density at a discount.</strong> The discount is $1/(1+v)$ with $v=\log a/\log P(a)-1$. Nearly-prime composites ($2p$, $3p$) have small $v$ and look dangerous. They cannot fill a slot: a packing argument caps their density at about $\sqrt{v}$ of the ambient territory.</li>
<li><strong>The tradeoff integrates to $\pi/4$.</strong> High conversion and tight packing pull in opposite directions. The worst case is a quarter-turn: a slot of composites realizes at most about $\frac\pi4\approx 0.785$ of a full-rate prime. Every odd prime beats its own composites.</li>
<li><strong>The prime $2$ is a separate estimate.</strong> If $2$ is in the set, the odd slots finish the job. If not, a direct bound gives $f(A)\lt 1.60\lt f(\mathcal{P})$. The napkin is the $\pi/4$. The paper is the error tracking that makes $0.901\lt 1$ for every odd prime, and the $1.60$ when $2$ is missing.</li>
</ol>
</div>
...
Unique factorization gives a lemma we will need when two territories try to occupy the same integer.
...
**Even perfect numbers** $\{6, 28, 496, 8128, \ldots\}$. Each has the Euclid–Euler form $2^{p-1}(2^p-1)$ with $2^p-1$ itself prime. The odd parts $2^p-1$ and $2^q-1$ divide one another only when $p$ divides $q$; for distinct primes $p,q$ that never happens, so these numbers do not divide one another.
...
Largest in what sense is the next section but one. Counting how many elements you have, or what proportion of the integers they occupy, is the wrong scale, and it is worth seeing why before we change scale.

The **natural density** of a set $S$, when the limit exists, is

$$
d(S) \;=\; \lim_{N \to \infty} \frac{\#\{ n \in S : n \le N \}}{N}.
$$

The even numbers have density $1/2$. A single interval $(x,2x]$ is a *finite* set, so its density is $0$ — it occupies about half the integers up to $2x$, and then as $N\to\infty$ it is diluted to nothing.

What one can do is string infinitely many such intervals together, with huge gaps between them. Pick $x_1 = 2$ and take $(2,4]$. Skip far enough that those three integers are a negligible fraction of the next $x_2$, then take $(x_2, 2x_2]$. Repeat. Two running proportions oscillate:
...
Besicovitch showed that by placing the intervals farther and farther out you can make the *upper* density of a primitive set arbitrarily close to $1/2$. Behrend and Erdős showed that the *lower* density is always $0$. Density oscillates too wildly to pick a winner. We need a scale that notices small integers more than large ones, and that stays finite even for infinite sets.
...
This is the Erdős sum. Each integer $a$ is given a positive weight that gets smaller as $a$ gets larger, but slowly. (Reminder: $\log$ is $\ln$, so $\log 6 \approx 1.792$, not $\log_{10} 6$.)
...
The first two primes already contribute more than $1$. The first eight already contribute $1.33$, against $0.161$ for $\{6,10,15\}$. Henri Cohen computed the full sum:
...
- Weighing by $1/a$ makes the primes score $\sum 1/p = \infty$. The numbers with $\Omega(n) = k$ score infinity too (the $k=2$ case is $\sum_{p\le q} 1/(pq)$, which contains $\frac12\bigl((\sum 1/p)^2 - \sum 1/p^2\bigr)$). The scale cannot tell the contestants apart.
...
- Weighing by $1/(a(\log a)^2)$ still lets the primes converge, but for a bad reason: the integral test already says $\sum_n 1/(n(\log n)^2)$ converges, so *every* set of integers has a finite score. Primitivity is no longer the constraint that keeps the sum finite, and the contest stops being about the divisibility structure.

The weight $1/(a\log a)$ sits on the knife-edge from the last section: $\sum_n 1/(n\log n)$ diverges, while $\sum_p 1/(p\log p)$ converges. The scale is sensitive enough to see the primes as a finite budget, but not so harsh that composites become irrelevant. Primitivity is exactly what makes the sum finite; that is Erdős 1935, and it is where the later proof begins.
...
Before asking which primitive set is heaviest, you need to know that every primitive set has a finite score, bounded by a constant that does not depend on the set. That is the 1935 theorem, and it is the next three sections.
...
We already defined natural density. The even numbers have density $1/2$. The multiples of $a$ have density $1/a$.
...
Now drop those whose cofactor $n/6$ is even (i.e. divisible by a prime $< 3$): $12=6\cdot 2$, $24=6\cdot 4$, $36=6\cdot 6$, $48=6\cdot 8$, $60=6\cdot 10$. What remains is $\{6, 18, 30, 42, 54\}$, five numbers, and $60/12 = 5$.
...
Thus $90 \in L_6 \cap L_{18}$. But $\{6, 18\}$ is *not* primitive, because $6 \mid 18$. That is the lemma in one direction: if the territories meet, one generator divides the other.

The other direction, a primitive pair whose territories do *not* meet: $\{6,10\}$. Here $P(6)=3 < 5=P(10)$. Suppose some $n = 6s = 10 s'$ were in both, with every prime of $s$ at least $3$ and every prime of $s'$ at least $5$. Then every prime of $6$ is $\le 3 < 5$, so $6$ shares no prime with $s'$. Euclid's lemma would force $6$ to divide $10$. It does not, so no such $n$ exists.

A same-largest-prime illustration, again not primitive: $\{9,27\}$. Both have $P=3$, and $27 = 9\cdot 3$ with cofactor $3 \ge 3$, so $27 \in L_9$. The smaller exponent of $3$ divides the larger. That is the second case of the proof.
...
Because the $L_a$ are disjoint, they cannot cover more than the whole number line. A catch: natural density is not a measure, so we cannot blindly add infinitely many densities. (The even numbers and the odd numbers each have density $1/2$, and their union has density $1=\tfrac12+\tfrac12$; that pair happens to add. An infinite family of densities need not.) We do not need to. For any *finite* subcollection of $A$, the corresponding $L_a$ are disjoint, the density of their union is the sum of their densities, and a subset of the integers cannot have density greater than $1$. Every finite sum of the $d(L_a)$ is therefore $\le 1$, and so the infinite sum is $\le 1$ as well:
...
How large is that product $\prod_{q < P(a)}(1-1/q)$? A heuristic first, then the named theorem.

Taking logarithms, $\prod_{p \le x}(1-1/p) \approx \exp\bigl(-\sum_{p \le x} 1/p\bigr)$, because $\log(1-1/p) \approx -1/p$ for large $p$. The sum of reciprocals of primes up to $x$ grows like $\log\log x$ (one logarithm slower than the harmonic series, because the primes are about $1/\log t$ dense). So the product is about
...
More generally, for any integer $n\ge 2$ write $A_n := A \cap L_n$. When $n=p$ is prime this recovers the slot above, because $L_p$ is exactly the integers whose smallest prime factor is $p$. Then $f(A) = \sum_p f(A_p)$, because the blocks $A_p$ do not overlap. The conjecture would follow if, for every prime $p$ and every primitive $A$,

$$
f(A_p) \;\le\; f(p) \;=\; \frac{1}{p\log p}.
$$

Simply: in the slot of numbers whose smallest prime factor is $p$, the single best choice is to take $\{p\}$ itself, not a primitive bunch of composites all divisible by $p$. A prime $p$ with this property (for every primitive $A$) is called **Erdős-strong**. If every prime were Erdős-strong, the primes would win.

A concrete odd slot, before any asymptotic. The slot $p=5$ consists of integers whose smallest prime factor is $5$. The triple $\{25, 35, 55\}$ lives there (check: $5^2$, $5\cdot 7$, $5\cdot 11$, and none divides another):

$$
f(25)+f(35)+f(55) \;\approx\; 0.0124+0.0080+0.0045 \;=\; 0.025,
$$

against $f(5)\approx 0.124$. The slot $p=3$, skipping $3$ itself and taking $\{9,15,21,33\}$:
...
The missing idea is to stop treating composites as if they were primes. Once we charge them a discount, $(\star)$ being tight stops mattering: a $21\%$ discount swallows the Mertens oscillation for every odd prime. (At $p=3$ the explicit error tracking inflates $\pi/4\approx 0.785$ up to $0.901$, still strictly less than $1$.) That is the next three sections.
...
A fully numerical window, on a dangerous $2p$. Take $a=22=2\cdot 11$. Then $v(22)=\log 2/\log 11\approx 0.289$, so $\sqrt{v}\approx 0.538$ and $1/\sqrt{v}\approx 1.86$. Here $a^*=2$ and $Q=2$, and the window is

$$
\bigl[2,\; 2^{1.86}\bigr) \;\approx\; [2,\; 3.62).
$$

The primes in that window are $2$ and $3$. Already the three multipliers $c=1,2,3$ contribute extra copy-density $1+\frac12+\frac13\approx 1.83$, matching $1/\sqrt{v}\approx 1.86$. Those copies still have to sit inside the ambient slot, so the original $L_{22}$ is allowed to spend only about $1/1.86 \approx 0.54$ of the slot — the packing $\sqrt{v}$ on the nose, in the case the 1935 bound was most afraid of.
...
Simply: **you should never skip an odd prime in favour of composites with that smallest prime factor.** Skipping $p$ and filling the slot with $2p$, $3p$, $p^2$, products of larger primes, whatever primitive combination you like, returns less than $90\%$ of what $p$ itself was worth.
...
We already know $\sum 1/(n(\log n)^2)$ converges, by the integral test in the infinite-sums section. By the **limit comparison test**, a series of positive terms that behaves like a convergent series converges. So $\sum_p 1/(p\log p)$ converges.
